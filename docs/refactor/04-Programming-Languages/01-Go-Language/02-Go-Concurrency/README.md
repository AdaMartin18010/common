...
**定义 2.1.2** (Channel通信)
Channel通信是一个四元组 $C = (P_1, P_2, M, \tau)$，其中：

- $P_1$ 是发送进程
- $P_2$ 是接收进程
- $M$ 是消息集合
- $\tau: P_1 \times M \rightarrow P_2$ 是通信函数

**定理 2.1.1** (通信安全性)
对于任意channel $c$ 和消息 $m$，如果 $c$ 是类型安全的，则 $\tau(p_1, m) = p_2$ 当且仅当 $m$ 的类型与 $c$ 的类型匹配。
...
**定义 2.2.1** (数据竞争)
数据竞争是一个三元组 $R = (T_1, T_2, V)$，其中：

- $T_1, T_2$ 是并发线程
- $V$ 是共享变量
- $T_1$ 和 $T_2$ 同时访问 $V$，且至少有一个是写操作
