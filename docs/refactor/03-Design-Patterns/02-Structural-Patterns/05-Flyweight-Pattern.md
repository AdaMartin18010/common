...
设 $$ I $$ 为内部状态集合，$$ E $$ 为外部状态集合，享元 $$ F $$ 定义为：

$$
F: I \times E \rightarrow \text{Object}
$$

对于任意内部状态 $$ i \in I $$ 和外部状态 $$ e \in E $$，享元函数 $$ F $$ 创建对象。
...
1. 设 $$ n $$ 为对象数量，$$ m $$ 为内部状态数量
2. 不使用享元模式：内存使用 = $$ O(n \times m) $$
3. 使用享元模式：内存使用 = $$ O(m) + O(n) $$
4. 当 $$ n \gg m $$ 时，享元模式显著减少内存使用
