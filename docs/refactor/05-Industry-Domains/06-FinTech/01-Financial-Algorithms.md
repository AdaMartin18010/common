...
**定义 1.1** (金融算法): 金融算法是一个四元组 $\mathcal{F} = (M, P, R, T)$，其中：

- $M$ 是市场模型集合 (Market Models)
- $P$ 是定价模型集合 (Pricing Models)
- $R$ 是风险模型集合 (Risk Models)
- $T$ 是交易模型集合 (Trading Models)
...
**定义 1.2** (现值): 现值 $PV$ 是未来现金流 $CF_t$ 在当前时点的价值：

$$PV = \sum_{t=1}^{n} \frac{CF_t}{(1 + r)^t}$$

其中 $r$ 是折现率，$n$ 是期数。

**定义 1.3** (终值): 终值 $FV$ 是当前投资在未来时点的价值：
